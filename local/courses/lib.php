...
// Sandeep - DM-174 Project Proposal is diplaying to student and teacher - Starts.
    ...
    if(is_siteadmin()){
        $ls_managereports .= html_writer::start_tag('li', array('id' => 'id_leftmenu_noticeboard', 
                        'class'=>'pull-left user_nav_div timetable'));
        $categories_url = new moodle_url('/blocks/learnerscript/managereport.php');
        $categories = html_writer::link($categories_url, '<span class="managereports_icon dypatil_cmn_icon icon"></span><span class="user_navigation_link_text">'.get_string('leftmenu_ls_managereports','local_courses').'</span>',array('class'=>'user_navigation_link'));
        $ls_managereports .= $categories;
        $ls_managereports .= html_writer::end_tag('li');
    }
    return array('5' => $coursecatnodes_course,'10' => $coursecatnodes_announcement,'8' => $coursecatnodes_program,'7' => $coursecatnodes,'11' => $coursecatnodes_queries,'12' => $coursecatnodes_timetable, '9' => $coursecatnodes_certificate, '10' => $ls_managereports);
}
